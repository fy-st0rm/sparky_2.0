...
namespace Sparky {
	...
	std::unordered_map<int, std::string> Log::usage_msg = {
		{ SPARKY_ASSIGN_SCENE, "[Usage]: use `switch_scene(const std::string& scene_name)` to switch to required scene.\n"}
	};
	...
	{
		std::cerr << Color::RED << "[Error]: " << msg << Color::DEFAULT << std::endl;
		Log::usage(tag);
		exit(1);
	}
	...
	{
		std::cout << Color::GREEN << "[Sucess]: " << msg << Color::DEFAULT << std::endl;
		Log::usage(tag);
	}
	
	void Log::warning(const std::string& msg, int tag)
	{
		std::cerr << Color::YELLOW << "[Warning]: " << msg << Color::DEFAULT << std::endl;
		Log::usage(tag);
	}
}
